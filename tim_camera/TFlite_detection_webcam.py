""" Python script for webcam object detection anmd distance calculation"""
...
# -----------------------------------------------------------------
# Average parameters
avg_width_person = 45+8+4 # +8 due to borders not aligning to body
avg_height_person = 172
avg_proportion_person = avg_width_person / avg_height_person
...
while True:
    ...
    for i in range(len(scores)):
        if ((scores[i] > min_conf_threshold) and (scores[i] <= 1.0)):
            ...
            if(labels[int(classes[i])] != "person" and labels[int(classes[i])] != "teddy bear" and labels[int(classes[i])] != "chair"):
                continue
            else:
                obj_type.append(str(labels[int(classes[i])]))
            ...
            vid_width = int(videostream.width)
            ...
            if(len(coords) > 1):
                ...
                for i in range(len(coords)):
                    ...
                    camera_distance.append((fokal_empir * avg_width_person) / proportion_x[i])
                    print("Distance obj "+str(i)+" ("+str(obj_type)+") - camera: "+str(camera_distance[i]))
                    
                    if(i>0):
                        # Calculate min dist (only horizontal)
                        if(obj_type[i] == "person"):
                            min_dist_x = proportion_x[i]/avg_width_person * 150
                        ...
                        for j in range(i):
                            ...
                            print("X-Distanz objekt i -> j: "+str(min_dist_obj_x_1)+" - X-Distanz obj j -> i: "+str(min_dist_obj_x_2)+" - minimale Distanz: "+str(min_dist_x))
                            print("Z-Distanz objekt i - j: "+str(dist_obj_z))
                            ...
                            if(min_dist_obj_x_1 < min_dist_obj_x_2):
                                ...
                                if(objects_distance < min_dist_x):
                                    print("AAAA "+str(objects_distance)+" j = "+obj_type[j])
                                ...
                                if(objects_distance < min_dist_x):
                                    print("BBB "+str(objects_distance)+" j = "+obj_type[j])
